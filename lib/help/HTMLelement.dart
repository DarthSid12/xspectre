...
class GlowingAnimatedBorder extends StatefulWidget {
  double height;
  double width;
  GlowingAnimatedBorder(this.height, this.width);
  @override
  _GlowingAnimatedBorderState createState() =>
      _GlowingAnimatedBorderState(height, width);
}

class _GlowingAnimatedBorderState extends State<GlowingAnimatedBorder>
    with TickerProviderStateMixin {
  AnimationController _resizableController;
  double height;
  double width;
  _GlowingAnimatedBorderState(this.height, this.width);
  static Color colorVariation(double note) {
    return Colors.blue.withOpacity(note);
  }
        ...
        builder: (context, snapshot) {
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: colorVariation(_resizableController.value), width: 10),
            ),
          );
        });
  }
...
Container videoElement(String url, String viewID, double height, double width) {
  IFrameElement someRandomHtmlElement = IFrameElement();
  url = url.toLowerCase().startsWith("www.")
      ? url
  ...
  someRandomHtmlElement.width = width.toString();
  ui.platformViewRegistry
      .registerViewFactory(viewID, (int viewId) => someRandomHtmlElement);
    ...
    child: Stack(
      children: [
        Container(
          ...
          width: width,
          decoration: BoxDecoration(
            border: Border.all(color: Color.fromRGBO(0, 0, 0, 0), width: 5),
          ),
          child: HtmlElementView(
            key: UniqueKey(),
            viewType: viewID,
        ...
        GlowingAnimatedBorder(height - 2, width - 2),
      ],
    ),
  );
}
